...
function request(path, { method = 'GET', headers = {}, body } = {}) {
  ...
  if (url.pathname.startsWith('/api/admin/') && ADMIN_KEY) {
    resolvedHeaders['x-api-key'] = ADMIN_KEY;
  }
...
async function main() {
  ...
  if (rawCmd === 'status') {
    ...
    console.log(await get('/api/admin/health'));
    return;
  }
  ...
  if (rawCmd === 'workflows') {
    ...
    if (rawSubCmd === 'list') {
      ...
      console.log(await get(`/api/admin/workflows?tenant=${encodeURIComponent(tenant)}`));
      return;
    }
