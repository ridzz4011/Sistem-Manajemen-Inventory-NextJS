...
`Cargo.lock` is intentionally not tracked in this client/example repo. This
example depends on local VFlow source paths (`../../../crates/...`) and should
be built from a checkout that actually contains those crates. Keeping a stale
lockfile here causes dependency scanners to report vulnerabilities for a server
binary that this repo cannot build by itself.
